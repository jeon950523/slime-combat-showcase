...
**실제 Unity Play Mode Game View · 1920×1080 · 3.6초**
...
GIF는 같은 원정에서 순차 캡처한 두 실제 전투 시점을 3.6초로 압축한 **전투 프리뷰 몽타주**입니다. 연출용 콘셉트 영상이나 완성 트레일러가 아니라, 현재 구현이 어떤 화면과 루프로 동작하는지를 보여 주기 위한 런타임 증거입니다.
...
**실제 Unity Play Mode Game View · 1920×1080 · 5.0초**

같은 R1 원정에서 적이 쓰러진 전투 상태를 거쳐, 시체 4개가 실제로 포식 처리되어 `포식 Lv.2`의 변이 선택이 열리는 흐름입니다. 세 장의 실제 카드 중 `공격력 증가 +15%`를 선택했고, 마지막 프레임은 선택이 적용된 뒤의 `흡수 4 · 포식 Lv.2 · 13 / 27` HUD 상태입니다.

이 클립은 포식 애니메이션, 레벨업 카드, 선택 적용 결과를 한눈에 보기 위해 실제 플레이 모드의 순차 캡처를 5초로 구성한 프리뷰 몽타주입니다. 아직 장시간 무편집 플레이 영상은 아니며, 원본 MP4가 준비되면 함께 추가합니다.
...
| README 미리보기 | 3.6초 전투 스냅샷 + 5초 포식·카드 선택 흐름, 1920×1080 GIF |
