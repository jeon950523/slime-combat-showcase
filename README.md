...
**실제 Unity Play Mode Game View · 960×540 · 5.2초 · 8fps 연속 캡처**
...
GIF는 실행 중인 Unity Game View를 프레임 단위로 연속 캡처한 40프레임입니다. 적 이동·피격 수치·처치 상태·남은 시간이 한 원정 안에서 계속 변하며, 정지 화면을 이어 붙인 몽타주가 아닙니다. 연출용 콘셉트 영상이나 완성 트레일러가 아니라, 현재 구현이 어떤 화면과 루프로 동작하는지를 보여 주기 위한 런타임 증거입니다.
...
## 5초 플레이 흐름 — 포식 → 변이 카드 선택

![시체 포식 후 포식 레벨업과 변이 카드 선택으로 이어지는 실제 플레이 흐름](assets/slime-gate-expedition-playflow.gif)

**실제 Unity Play Mode Game View · 960×540 · 5.2초 · 8fps 연속 캡처**

같은 R1 원정에서 전투가 진행되다가 시체 2개가 실제 포식 처리되어 `포식 Lv.2`의 변이 선택이 열리는 흐름입니다. 세 장의 실제 카드 중 `최대 체력 증가 +20%`를 선택했고, 마지막 프레임은 선택이 적용된 뒤의 `HP 578 / 578 · 흡수 2 · 포식 Lv.2 · 1 / 27` HUD 상태입니다.

포식 처리와 카드 선택은 포트폴리오 데모 실행에서 실제 게임 시스템과 실제 선택 UI를 통해 발생시켰습니다. GIF는 그 실행을 40프레임으로 끊김 없이 기록합니다. 아직 장시간 무편집 플레이 영상은 아니며, 원본 MP4가 준비되면 함께 추가합니다.
...
| README 미리보기 | 5.2초 전투 + 5.2초 포식·카드 선택, 각각 40프레임·960×540 연속 GIF |
...
1. 전투 원본 MP4 추가 (README에는 최신 게이트 원정 GIF 프리뷰를 유지)
